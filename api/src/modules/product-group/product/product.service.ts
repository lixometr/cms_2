...
export class ProductService extends ServiceBlueprint<Product> {
  ...
  constructor(
    ...
    super(productRepository, eventEmiter)
  }
  ...
  async findByCategoryFullSlug(
    { slug }: { slug: SLUG },
    payload: RequestPayload,
  ) {
    const category = await this.categoryService.findByFullSlug(
      { slug },
      payload,
    )
    if (!category)
      throw new BadRequestException('Category with such slug not found')
  ...
  async findAllWithFilters({}, payload: RequestPayload) {
    await this.eventEmiter.emitAsync(
      `${this.name}.${EventName.beforeFindAll}`,
      { payload },
    )
    const result = await this.productRepository.findAllWithFilters({}, payload)
    await this.eventEmiter.emitAsync(`${this.name}.${EventName.afterFindAll}`, {
      result,
      payload,
    })
    return result
  ...
  async getItemInfo(
    { id, info }: { id: ID; info: ProductInfoDto },
    payload: RequestPayload,
  ) {
