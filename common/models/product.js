module.exports = class Product {
    ...
    getNormOptions() {
        ...
        optIds.forEach((optId) => {
            const option = this.productOptions.find((opt) => opt.id.toString() === optId.toString());
            // if(!option) return
            ...
            if (type === "range") {
                ...
                values = option.values.filter((val) =>
                    this.activeOptions[optId].includes(val.id.toString())
                );
            }
    ...
    getActiveDate() {
        const activeDate = this.dates.find(
            (date) => date.id.toString() === this.activeDateId
        );
        return activeDate || {};
    }
    getPrice() {
    ...
    getActiveVariation() {
        return (
            this.variations.find(
                (variation) => variation.attrValue.id.toString() === this.activeVariationId.toString()
            ) || {}
        );
    }
    getProductOptions() {
